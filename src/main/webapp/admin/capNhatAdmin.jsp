  ...
  <div class="row">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="container_form">
        <form class="form-container" action="../admin?action=updateAdmin&customerId=${updateCustomer.id}" method="post">
          <h2 class="text-center">Cập nhật thông tin Admin</h2>

          <div class="mb-3">
            <label for="fullName" class="form-label">Tên Khách Hàng</label>
            <input type="text" class="form-control" id="fullName"
                   name="fullName" placeholder="Nhập tên khách hàng" value="${updateCustomer.fullName}">
          </div>
          <div class="mb-3">
            <label for="email" class="form-label">Email</label>
            <input type="email" class="form-control" id="email"
                   name="email" placeholder="Nhập email" value="${updateCustomer.email}">
          </div>
          <div class="mb-3">
            <label for="numberPhone" class="form-label">Số Điện Thoại</label>
            <input type="number" class="form-control" id="numberPhone"
                   name="numberPhone" placeholder="Nhập số điện thoại" value="${updateCustomer.numberPhone}">
          </div>
          <div class="mb-3">
            <label for="role" class="form-label">Quyền Hạn</label>
            <select class="form-control" id="role" name="role">
              <option value="false" ${updateCustomer.role == false ? 'selected' : ''}>Người Dùng</option>
              <option value="true" ${updateCustomer.role == true ? 'selected' : ''}>Admin</option>
            </select>
          </div>
          <div class="mb-3">
            <button type="submit" class="btn btn-primary width-btn">Cập Nhật Admin</button>
          </div>
        </form>
